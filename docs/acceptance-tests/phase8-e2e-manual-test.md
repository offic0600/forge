...
| # | 旅程 | TC | 现象 | 预期 | 实际 | 严重度 | 状态 |
|---|------|-----|------|------|------|--------|------|
| 1 | 1 | TC-1.2 | Sidebar 无 Evaluations 导航 | Sidebar 应包含 Evaluations 链接 | navItems 缺少 /evaluations 条目 | Medium | ✅ 已修复 (BF-1.1) |
| 2 | 3 | TC-3.1 | 创建文件夹后出现 .gitkeep 文件 | 创建文件夹不应显示 .gitkeep | 文件树中多出 .gitkeep | Low | ✅ 已修复 (BF-1.2) |
| 3 | 4 | TC-4.2 | 离开 workspace 再返回后，用户发言显示为 AI 发言 | 用户消息应靠右显示（用户气泡样式） | 用户消息显示为 AI 气泡样式（靠左） | High | ✅ 已修复 (BF-2.1) |
| 4 | 4 | TC-4.4 | AI 回复内容未按时间顺序展示 | 流式文本+工具调用应按时间交错展示 | 文本在上、工具在中、日志在下（分块展示） | Medium | ✅ 已修复 (BF-3.1) |
| 5 | 4 | TC-4.5 | Intent Confirmation 选 Profile 后直接执行 | 选 Profile 后应追问具体意图再执行 | 选择后直接执行，AI 仍不知道用户真正想做什么 | Medium | ✅ 已修复 (BF-3.2) |
| 6 | 4 | TC-4.7 | workspace_list_files 只返回文件夹+文件个数 | 用户期望看到所有文件名列表 | 只显示目录摘要（文件夹名+数量） | Low | 不修复（AI 摘要行为） |
| 7 | 4 | TC-4.10 | Focus Mode 切换时页面显示异常 | 切换 Focus Mode 布局应正常过渡 | 页面显示错乱（见截图 bug25） | Medium | ✅ 已修复 (BF-1.3) |
| 8 | 5 | TC-5.3 | Skills Tab 无启用/禁用开关 | 每个 Skill 应有 toggle 开关 | 只展示列表，无法切换状态 | Medium | ✅ 已修复 (BF-2.2) |
| 9 | 8 | TC-8.1 | Evaluations 页面数据全部不可交互 | 时间选择器/图表/表格应可操作 | 页面为纯静态展示，无交互响应 | Medium | ✅ 已修复 (BF-2.3) |
